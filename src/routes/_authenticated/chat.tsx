...
function ChatLayout() {
  ...
  useEffect(() => {
    if (!threadId && data) {
      if (items.length)
        navigate({ to: "/chat/$threadId", params: { threadId: items[0].id }, replace: true });
    }
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden">
      {/* Sidebar */}
      <aside
        className={`${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 fixed md:static z-40 inset-y-0 left-0 w-72 bg-sidebar text-sidebar-foreground border-r border-sidebar-border flex flex-col transition-transform`}
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2">
          ...
          {items.map((c) => (
            <div
              key={c.id}
              className={`group flex items-center rounded-md ${threadId === c.id ? "bg-sidebar-accent text-sidebar-accent-foreground" : "hover:bg-sidebar-accent/50"}`}
            >
              <Link
                to="/chat/$threadId"
          ...
          {!items.length && (
            <p className="text-xs text-muted-foreground px-3 py-4">Sem conversas ainda.</p>
          )}
        </div>
