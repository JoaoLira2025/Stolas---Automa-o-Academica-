...
export const sendMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const reply = await chatCompletion({ messages, model: "openrouter/free" });
...
Se o material de origem tiver marcadores de nota de rodapé estilo Wikipedia (ex.: [1], [2], [9]), REMOVA esses números do texto — eles não correspondem a uma lista de referências no padrão ABNT e ficariam órfãos no documento final. Quando o material não fornecer autor e ano identificáveis para uma citação, apenas mantenha a informação no texto sem nenhum marcador numérico ou use (Doc: nome_do_arquivo) para indicar a fonte, conforme já é feito no prompt de chat..`,
          },
          { role: "user", content: userPrompt },
        ],
        model: "openrouter/free",
